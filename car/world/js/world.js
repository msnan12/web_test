...
class World {
    ...
    generateCorridor(start,end,extendEnd=false){
        ...
        const tmpEnvelopes=segs.map(
            (s) => new Envelope(s, this.roadWidth, this.roadRoudness)
        );
        if(extendEnd){
            segs.pop();
        }
    ...
    draw(ctx, viewPoint, showStartMarkings=true, renderRadius=2000) {
        ...
        // マーキング
       for(const markig of this.markings){
            if(!(markig instanceof Start) || showStartMarkings){
                markig.draw(ctx);
            }
       }
        // 中心線
